...
// --- FUNÇÃO HELPER DE DATA (PARA CSV) ---
const formatarDataParaCSV = (data) => {
    if (!data) return '';
    const d = new Date(data);
    // Usamos UTC para evitar problemas de fuso horário
    const dia = String(d.getUTCDate()).padStart(2, '0');
    const mes = String(d.getUTCMonth() + 1).padStart(2, '0'); // Meses são 0-indexados
...
// ***** ROTA DE EXPORTAÇÃO CSV (ESTOQUE) *****
...
                { key: 'valor_total', header: 'Valor Total em Estoque (R$)' },
...
            let status = 'OK';
            if (item.estoqueminimo > 0 && item.totalunidades <= item.estoqueminimo) {
                status = 'CRITICO';
            } else if (item.tipo_unidade === 'cartela' && item.totalunidades < 5000 && item.totalunidades > 0) {
                status = 'BAIXO';
            }
